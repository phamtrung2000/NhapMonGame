...
void Grid::UpdateGrid(vector<LPGAMEOBJECT> listObj)
{
	...
	int right = (int)(ceil(((Camera::GetInstance()->cam_x + 252) / CELL_WIDTH)));
	int top = (int)(Camera::GetInstance()->cam_y / CELL_HEIGHT);
	int bottom = (int)(ceil(((Camera::GetInstance()->cam_y + 230) / CELL_HEIGHT)));
	...
	{
		float l, t, r, b;
		if (listObj[i]->ObjType != OBJECT_TYPE_BRICK)
		{
		...
		int Right = (int)ceil(r / CELL_WIDTH);
		int Bottom = (int)ceil(b / CELL_HEIGHT);
		//DebugOut(L"Left%d\n", Left);
		//DebugOut(L"Top%d\n", Top);
		//DebugOut(L"Right%d\n", Right);
		//DebugOut(L"Bottom%d\n", Bottom);
		if (listObj[i]->canDelete == false && CheckObjPos(listObj[i]->x, listObj[i]->y))
		{
...
void Grid::GetListObj(vector<LPGAMEOBJECT>& listNotMoveObj, vector<LPGAMEOBJECT>& listEnemy, vector<LPGAMEOBJECT>& listObj)
{
	...
	for (int i = left; i < right; i++)
	{
		for (int j = top; j < bottom; j++)
		{
			...
			for (unsigned int k = 0; k < Cell[i][j].size(); k++)
			{
